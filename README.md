...
<body class="bg-slate-100 font-sans text-gray-800">
    ...
    <div class="container mx-auto p-4 max-w-5xl">
        ...
        <div id="dashboard-section" class="hidden-section">
            ...
            <div class="flex flex-wrap gap-2 mb-6 border-b pb-2">
                ...
                <button onclick="switchTab('factory')" class="px-4 py-2 bg-gray-200 text-gray-700 rounded font-medium tab-btn" id="btn-factory">🏭 Point Factory</button>
            ...
            <!-- TAB 3: POINT FACTORY (NEW FEATURE) -->
            <div id="tab-factory" class="hidden-section space-y-6 max-w-xl mx-auto bg-white p-6 rounded-lg shadow-md border-t-4 border-indigo-600">
                <h3 class="text-xl font-bold text-indigo-800 mb-2">🏭 Point Factory (Futkar Withdrawal)</h3>
                <p class="text-sm text-gray-600 mb-4">Admin dwara active ki gayi limit ke mutabiq aap yahan futkar mein (jaise 5, 10, 8) points nikal sakte hain. Limit se zyada points nahi milenge!</p>
                
                <div class="bg-indigo-50 p-4 rounded-lg border border-indigo-200 mb-4 space-y-2">
                    <p class="text-sm font-semibold text-indigo-900">Admin Active Limit: <span id="factory-limit-val" class="text-emerald-700 text-base font-bold">0</span> Pts</p>
                    <p class="text-sm font-semibold text-indigo-900">Abhi tak Nikale Gaye Points: <span id="factory-withdrawn-val" class="text-amber-600 text-base font-bold">0</span> Pts</p>
                    <p class="text-sm font-semibold text-indigo-900">Bachi Hui Limit (Baaki): <span id="factory-remaining-val" class="text-blue-700 text-base font-bold">0</span> Pts</p>
                </div>

                <div class="space-y-3">
                    <label class="block text-sm font-medium">Kitne Points nikalne hain? (Futkar mein):</label>
                    <input type="number" id="factory-claim-input" placeholder="e.g. 5, 10, 8" class="w-full p-2 border rounded focus:ring-2 focus:ring-indigo-500">
                    <button onclick="claimFactoryPoints()" class="w-full bg-indigo-600 text-white py-2 rounded font-bold hover:bg-indigo-700">Withdraw Points from Factory</button>
                </div>
            </div>
            ...
            <div id="tab-admin" class="hidden-section space-y-6">
                <div class="bg-white p-6 rounded-lg shadow-md border-t-4 border-emerald-600">
                    ...
                    <!-- NEW: SET USER FACTORY POINT LIMIT -->
                    <div class="border-b pb-6 mb-6 bg-slate-50 p-4 rounded border border-slate-200">
                        <h4 class="font-semibold mb-3 text-lg text-emerald-900">⚡ Set User Factory Point Limit (Admin Control)</h4>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
                            <div>
                                <label class="block text-sm font-medium">Target User Mobile Number:</label>
                                <input type="text" id="admin-target-phone" placeholder="Enter User Phone Number" class="w-full p-2 border rounded bg-white">
                            </div>
                            <div>
                                <label class="block text-sm font-medium">Active Limit (e.g. 40 or more):</label>
                                <input type="number" id="admin-target-limit" placeholder="40" class="w-full p-2 border rounded bg-white">
                            </div>
                        </div>
                        <button onclick="adminSetFactoryLimit()" class="bg-emerald-700 text-white px-4 py-2 rounded font-bold hover:bg-emerald-800 text-sm">Activate Limit For User</button>
                    </div>
    ...
    <!-- JavaScript Logic -->
    <script>
        const DB_KEY = "cricket_portal_db_v11";
        ...
        function getDB() {
            ...
            if(!parsed.factoryLimits) parsed.factoryLimits = {};
            if(!parsed.factoryWithdrawn) parsed.factoryWithdrawn = {};
            return parsed;
        }
        ...
        function switchTab(tabName) {
            ...
            ['matches', 'points', 'factory', 'subscription', 'addcoins', 'admin'].forEach(t => {
        ...
        // --- ADMIN: SET FACTORY LIMIT ---
        function adminSetFactoryLimit() {
            let db = getDB();
            let phone = document.getElementById("admin-target-phone").value.trim();
            let newLimit = parseInt(document.getElementById("admin-target-limit").value);

            if (!phone || isNaN(newLimit)) {
                alert("Kripya sahi phone number aur limit value dalein!");
                return;
            }

            db.factoryLimits[phone] = newLimit;
            if(db.factoryWithdrawn[phone] === undefined) {
                db.factoryWithdrawn[phone] = 0;
            }
            saveDB(db);
            alert(`Success! Number ${phone} ke liye factory limit update karke ${newLimit} points kar di gayi hai.`);
            document.getElementById("admin-target-phone").value = "";
            document.getElementById("admin-target-limit").value = "";
        }

        function updateFactoryUI() {
            let db = getDB();
            let phone = currentUserPhone;
            let limit = db.factoryLimits[phone] || 0;
            let withdrawn = db.factoryWithdrawn[phone] || 0;
            let remaining = limit - withdrawn;
            if(remaining < 0) remaining = 0;

            document.getElementById("factory-limit-val").innerText = limit;
            document.getElementById("factory-withdrawn-val").innerText = withdrawn;
            document.getElementById("factory-remaining-val").innerText = remaining;
        }

        function claimFactoryPoints() {
            let db = getDB();
            let phone = currentUserPhone;
            let limit = db.factoryLimits[phone] || 0;
            let withdrawn = db.factoryWithdrawn[phone] || 0;
            ...
            let claimAmt = parseInt(document.getElementById("factory-claim-input").value);

            if (isNaN(claimAmt) || claimAmt <= 0) {
                alert("Kripya nikalne ke liye valid points darj karein!");
                return;
            }

            if (limit === 0) {
                alert("Aapke number par koi factory limit active nahi hai! Admin se contact karein.");
                return;
            }

            if (claimAmt > remaining) {
                alert(`Error: Aapki bachi hui limit sirf ${remaining} Points ki hai! Aap isse zyada nahi nikal sakte.`);
                return;
            }

            db.factoryWithdrawn[phone] = withdrawn + claimAmt;
            db.users[phone].wallet += claimAmt;
            if(db.userIDs[db.users[phone].userid]) {
                db.userIDs[db.users[phone].userid].wallet = db.users[phone].wallet;
            }

            saveDB(db);
            alert(`Badhai ho! Factory se ${claimAmt} Points safaltapurvak aapke wallet mein add ho gaye hain.`);
            document.getElementById("factory-claim-input").value = "";
            initDashboard();
            updateFactoryUI();
        }
